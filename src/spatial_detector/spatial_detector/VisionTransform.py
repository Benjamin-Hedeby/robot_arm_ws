...
def transform_camera_to_base(cam_x, cam_y, cam_z, current_joints, imu_roll=None, imu_pitch=None):
    ...
    P_cam = np.array([cam_x-0.01, cam_y+0.01, cam_z, 1.0])
    ...
    if imu_roll is not None and imu_pitch is not None:
        # Build a new, reality-corrected rotation matrix
        R_corrected = euler_to_rot_matrix(imu_roll, imu_pitch, 0)
        
        # Overwrite the top-left 3x3 rotation block in the transformation matrix
        T_tool_cam[:3, :3] = R_corrected
